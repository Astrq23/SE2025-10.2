...
## 2. Goals & Objectives (Mục đích & Mục tiêu)

### 2.1. Goals (Mục đích chiến lược)
1.  **Seamless User Experience (UX):** Xây dựng giao diện "Dark Glassmorphism" hiện đại, loại bỏ cảm giác khô khan của các dApp truyền thống, giúp người dùng thao tác dễ dàng như Web2.
...
### 3.1. Frontend Implementation (Chi tiết Giao diện)
...
### 3.2. Backend Implementation (Chi tiết Blockchain)
Hệ thống Smart Contract viết bằng **Solidity 0.8.20**, bao gồm 5 contracts chính tương tác chặt chẽ với nhau:

1.  **ZenithToken (ZEN):** Token quản trị chuẩn ERC-20, tích hợp quyền `operationAddress` cho các tác vụ hệ thống.
2.  **TokenSwap (DEX):** Contract xử lý thanh khoản và quy đổi token.
    * *Logic:* Thu phí 0.3% cho mỗi giao dịch Swap.
3.  **TokenStaking:** Contract quản lý tiền gửi.
    * *Logic:* Tính toán lãi suất dựa trên từng giây (`block.timestamp`), chính xác tuyệt đối.
4.  **ZenithNFT & NFTMarketplace:** Hệ sinh thái vật phẩm số.
    * *Logic:* Listing NFT, khớp lệnh mua bán và tự động cắt 2.5% phí sàn cho Admin.

---

## 4. Key Challenges & Solutions (Vấn đề & Giải pháp)

Đây là phần thể hiện năng lực giải quyết vấn đề kỹ thuật (Technical Problem Solving):

### Về Frontend (Giao diện & Tích hợp)
...
>>>>>>> 9d89c8a2f39f9a9791f03e387dd59a89bc326309
